...
export function RoutesManagement({ routes, accessToken, onRefresh }: RoutesManagementProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {showForm && (
        <Card>
          <h3 className="mb-4">{editingRoute ? 'Edit Rute' : 'Tambah Rute Baru'}</h3>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex gap-3">
              <Button type="submit" disabled={loading}>
                {loading ? 'Menyimpan...' : editingRoute ? 'Update Rute' : 'Tambah Rute'}
              </Button>
      ...
      {routes.length === 0 ? (
        <Card>
          <p className="text-center text-gray-600 py-8">Belum ada rute tersedia</p>
        </Card>
      ) : (
                    ...
                    <div>
                      ...
                      <span
                        className={`px-3 py-1 rounded-full text-sm
                          ${route.isActive ? 'bg-success/10 text-success' : 'bg-gray-200 text-gray-600'}
                        `}
                      >
