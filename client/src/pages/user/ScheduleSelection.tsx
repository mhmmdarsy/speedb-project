...
export function ScheduleSelection({ route, schedules, onSelectSchedule, onBack }: ScheduleSelectionProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Date Selection */}
      <div>
        <label className="block mb-3 text-gray-900">Pilih Tanggal Keberangkatan</label>
            ...
            return (
              ...
              >
                <div className="text-center">
                  <p className={`text-sm ${isSelected ? 'text-primary' : 'text-gray-600'}`}>
                    {formatted.day}
                  </p>
      ...
      {selectedDate && (
        <div>
          <label className="block mb-3 text-gray-900">Pilih Jam Keberangkatan</label>
          {filteredSchedules.length === 0 ? (
            <Card>
              <p className="text-center text-gray-600">Tidak ada jadwal tersedia untuk tanggal ini</p>
            </Card>
          ) : (
                ...
                return (
                  <button
                    ...
                    className={`w-full p-5 rounded-xl border-2 transition-all duration-200 text-left
                      ...
                      ${!isSelected && isAvailable ? 'border-gray-200 bg-white hover:border-primary/50' : ''}
                      ${!isAvailable ? 'border-gray-200 bg-gray-100 opacity-50 cursor-not-allowed' : ''}
                    `}
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <Clock className={`w-6 h-6 ${isSelected ? 'text-primary' : 'text-gray-600'}`} />
                    ...
                    {!isAvailable && (
                      <p className="mt-2 text-sm text-error">Kursi tidak cukup untuk {passengers} penumpang</p>
                    )}
                  </button>
                );
              })}
